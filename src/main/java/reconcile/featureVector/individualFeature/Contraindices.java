...
private boolean nonCopularVerb(Annotation dep, String text)
{
  ...
  return dep.getType().startsWith(VERB) && !SyntaxUtils.isCopular(Annotation.getAnnotText(dep, text));
}
...
public String produceValue(Annotation np1, Annotation np2, Document doc, Map<Feature, String> featVector)
{
  ...
  if (featVector.get(DataConstructor.createFeature(Constants.APPOSITIVE)).equals(COMPATIBLE)) return COMPATIBLE;
  if (featVector.get(DataConstructor.createFeature("Prednom")).equals(COMPATIBLE)) return COMPATIBLE;
  ...
  if (!FeatureUtils.isPronoun(np2, doc)) {// notPronouns){
    ...
    if (depPath.size() == 3) {
      if (SyntaxUtils.isPreposition(depPath.get(1).getType())) {
        ...
        // text)))
        if (!DataConstructor.createFeature("Quantity").getValue(np1, np2, doc, featVector)
