...
public String produceValue(Annotation np1, Annotation np2, Document doc, Map<Feature, String> featVector)
{
  if (DataConstructor.createFeature(Constants.APPOSITIVE).getValue(np1, np2, doc, featVector).equals(COMPATIBLE))
    return COMPATIBLE;
  if (DataConstructor.createFeature(Constants.ALIAS).getValue(np1, np2, doc, featVector).equals(COMPATIBLE))
    return COMPATIBLE;
  if (DataConstructor.createFeature(Constants.PREDNOM).getValue(np1, np2, doc, featVector).equals(COMPATIBLE))
    return COMPATIBLE;
