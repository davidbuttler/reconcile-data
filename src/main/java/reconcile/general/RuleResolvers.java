      ...
      (isAppositive(np1, np2, doc) || isPrednom(np1, np2, doc))) {
        ...
        if (hn1.equalsIgnoreCase(hn2) && compModifier(np1, np2, doc) && sentNum(np1, np2, doc) <= 2) {
          ...
          if (NPSemanticType.getValue(np1, doc).equals(NPSemanticType.getValue(np2, doc))
              && !NPSemanticType.getValue(np1, doc).equals(NPSemTypeEnum.UNKNOWN)
...
private static Annotation ruleResolvePronounI(Annotation np2, Annotation[] basenps, int num, Document doc)
{
  ...
  if (quote2 < 0) {
    ...
    for (int i = basenps.length - 1; i >= 0; i--) {
      Annotation np1 = basenps[i];
      NPSemTypeEnum type = ProperNameType.getValue(np1, doc);
      if (type != null && type.equals(NPSemTypeEnum.PERSON)) {
        union(i, num);
        return np1;
      }
    }
...
{
  if (DataConstructor.createFeature("ProperName").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
  ...
  if (DataConstructor.createFeature("Span").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.INCOMPATIBLE)) {
    if (DEBUG) {
...
{
  return DataConstructor.createFeature("Appositive").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}

private static boolean isQuantity(Annotation np1, Annotation np2, Document doc)
{
  return DataConstructor.createFeature("Quantity").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}

private static boolean isIAntes(Annotation np1, Annotation np2, Document doc)
{
  return DataConstructor.createFeature("IAntes").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}

private static boolean isWeAntes(Annotation np1, Annotation np2, Document doc)
{
  return DataConstructor.createFeature("WeAntes").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}

private static boolean isNumberIncompatible(Annotation np1, Annotation np2, Document doc)
{
  String number = DataConstructor.createFeature("Number").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2));
  // if(DEBUG)
...
{
  String gender = DataConstructor.createFeature("Gender").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2));
  if (DEBUG) {
...
{
  String animacy = DataConstructor.createFeature("Animacy").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2));
  if (DEBUG) {
...
{
  return DataConstructor.createFeature("Alias").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}

private static boolean isPNStr(Annotation np1, Annotation np2, Document doc)
{
  return DataConstructor.createFeature("PNStr").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}

private static boolean isWordsStr(Annotation np1, Annotation np2, Document doc)
{
  return DataConstructor.createFeature("WordsStr").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}
...
private static boolean compModifier(Annotation np1, Annotation np2, Document doc)
{
  ...
  return DataConstructor.createFeature("Modifier").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}

private static boolean isWNClassComp(Annotation np1, Annotation np2, Document doc)
{
  String wnclass = DataConstructor.createFeature("WordNetClass").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2));
  if (DEBUG) {
...
{
  String proComp = DataConstructor.createFeature("ProComp").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2));
  if (DEBUG) {
...
{
  String syntax = DataConstructor.createFeature("Syntax").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2));
  if (DEBUG) {
...
{
  String pnsubstr = DataConstructor.createFeature("PNSubstr").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2));
  // if(DEBUG)
...
{
  return DataConstructor.createFeature("Title").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.INCOMPATIBLE);
}

private static boolean isPrednom(Annotation np1, Annotation np2, Document doc)
{
  return DataConstructor.createFeature("Prednom").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)).equals(
      NominalFeature.COMPATIBLE);
}
...
{
  return Double.parseDouble(DataConstructor.createFeature("WordNetDist").getValue(np1, np2, doc,
...
{
  return Integer.parseInt(DataConstructor.createFeature("SentNum").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)));
}
